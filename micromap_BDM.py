...
if __name__ == '__main__':
    ...
    if transm_use == 0:
        ...
        mod_tmax = smf.mixedlm('Tmax ~ Tmax_meteo * trans_Tmax + topo_index + topo_wetness + vegh + aspect_n + slope',
                               dfs_model, groups=dfs_model['region'])
    ...
    for site in site_names:
        ...
        for scenario in numbers_sc:
            ...
            svf_file = Path('/home/malle/transm_calcs/' + site + '/Output_CR_10m_' + forest_scenario + '/Plots/SVF.tif')
            ...
            if forest_scenario == 'summer_075_fm1_0mbuffer' or forest_scenario == 'summer_025_fm1_0mbuffer' or \
                    forest_scenario == 'summer_050_fm1_0mbuffer' or forest_scenario == 'summer':
            ...
            for date_num in range(1, 1529, 1):
                ...
                str_match = list(filter(lambda x: date_int[1:3] in str(x), transm_all))
                ...
                searchstring = '_EUR11_'+sce+'_QMgrid_summer10yrs'+str(date_num+1)+'bdm'+site[-1:]+'.tif'
                ...
                id_array_tmax = np.transpose(np.array(np.meshgrid(range(stack_tmax.shape[1]),
                                                                  range(stack_tmax.shape[2]))).T.reshape(-1, 2), (1, 0))
                ...
                combo_all_tmean = np.zeros((stack_tmean.shape[0] + id_array_tmean.shape[0], id_array_tmean.shape[1]),
                                           dtype=float)
                ...
                if transm_use == 1:
                    ...
                    micro_map_out_tmax = list(zip(micro_map_out_tmax_comb.astype(np.int16), idx_tmax.astype(np.int16),
                                                  idy_tmax.astype(np.int16)))
                    micro_map_out_tmean = list(zip(micro_map_out_tmean_comb.astype(np.int16), idx_tmax.astype(np.int16),
                                                   idy_tmax.astype(np.int16)))
                    micro_map_out_tmin = list(zip(micro_map_out_tmin_comb.astype(np.int16), idx_tmax.astype(np.int16),
                                                  idy_tmax.astype(np.int16)))
                else:
                ...
                if transm_use == 0:
                    ...
                    out_folder_tmax = Path('/home/malle/slfhome/Postdoc2/experiment_sites_select/' + site +
                                           '/MicroMaps/incl_transm/' + model + '/tmax')
                    out_folder_tmean = Path('/home/malle/slfhome/Postdoc2/experiment_sites_select/' + site +
                                            '/MicroMaps/incl_transm/' + model + '/tmean')
                    out_folder_tmin = Path('/home/malle/slfhome/Postdoc2/experiment_sites_select/' + site +
                                           '/MicroMaps/incl_transm/' + model + '/tmin')
                ...
                out_micromap_plot_tmax = out_folder_tmax / Path('microtemp_' + sce + '_' +
                                                                real_date.strftime('%-y-%-m-%-d')+'.png')
                out_micromap_plot_tmean = out_folder_tmean / Path('microtemp_' + sce+'_' +
                                                                  real_date.strftime('%-y-%-m-%-d')+'.png')
                out_micromap_plot_tmin = out_folder_tmin / Path('microtemp_' + sce + '_' +
                                                                real_date.strftime('%-y-%-m-%-d')+'.png')

                driver = gdal.GetDriverByName('GTiff')
                driver.Register()
                # create new clipped temp file:
                out_ds = driver.Create(str(out_micromap_map_tmax), micro_map_tmax.shape[1], micro_map_tmax.shape[0], 1,
                ...
                # TMEAN
                out_ds = driver.Create(str(out_micromap_map_tmean), micro_map_tmean.shape[1], micro_map_tmean.shape[0],
                ...
                # TMIN
                out_ds = driver.Create(str(out_micromap_map_tmin), micro_map_tmin.shape[1], micro_map_tmin.shape[0], 1,
                                       gdal.GDT_Int16, options=['COMPRESS=ZSTD', 'PREDICTOR=2', 'ZSTD_LEVEL=1'])
                out_ds.SetGeoTransform(gt)
                out_ds.SetProjection(proj)
                outband = out_ds.GetRasterBand(1)
